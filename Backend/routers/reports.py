...
from auth import ensure_auth_tables, require_auth_user
from database import execute_mutation, execute_query
from schemas import CustomReportCreate, CustomReportRecord, ReportFilters, ReportSummary, ReportValue
...
def _deserialize_filters(filters_value: str | dict | None) -> dict:
    ...
    if not filters_value:
        return ReportFilters().model_dump()
    try:
...
def _build_custom_report_record(row: dict) -> CustomReportRecord:
    ...
    return CustomReportRecord(
        ...
        filters=ReportFilters(**_deserialize_filters(row.get("filters"))),
...
async def list_custom_reports(request: Request) -> List[CustomReportRecord]:
    ...
    rows = await execute_query(
        """
        SELECT id, userId, title, chartType, metric, layoutSpan, sortOrder, filters, createdAt, updatedAt
        FROM custom_reports
        WHERE userId = %s
...
async def create_custom_report(request: Request, payload: CustomReportCreate) -> CustomReportRecord:
    ...
    inserted_id = await execute_mutation(
        """
        INSERT INTO custom_reports (userId, title, chartType, metric, layoutSpan, filters, createdAt, updatedAt)
        VALUES (%s, %s, %s, %s, %s, %s, NOW(), NOW())
        """,
        [actor["id"], payload.title.strip(), payload.chartType, payload.metric, payload.layoutSpan, _serialize_filters(payload.filters)],
    )
...
async def update_custom_report(reportId: int, request: Request, payload: CustomReportCreate) -> CustomReportRecord:
    ...
    await execute_mutation(
        ...
        WHERE id = %s AND userId = %s
        """,
        [payload.title.strip(), payload.chartType, payload.metric, payload.layoutSpan, payload.sortOrder, _serialize_filters(payload.filters), reportId, actor["id"]],
    )
