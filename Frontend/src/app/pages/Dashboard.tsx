...
import { Briefcase, Building2, FolderKanban, TrendingUp, AlertCircle, CheckCircle } from "lucide-react";
...
export function Dashboard() {
  const casesByStatus = [
    { id: "dash-status-open", name: "Open", value: cases.filter(c => c.status === "Open").length },
    { id: "dash-status-progress", name: "In Progress", value: cases.filter(c => c.status === "In Progress").length },
    { id: "dash-status-escalated", name: "Escalated", value: cases.filter(c => c.status === "Escalated").length },
    { id: "dash-status-closed", name: "Closed", value: cases.filter(c => c.status === "Closed").length },
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={casesByStatus}>
              <CartesianGrid key="dash-status-grid" strokeDasharray="3 3" stroke="#E5E7EB" />
              <XAxis key="dash-status-xaxis" dataKey="name" stroke="#6B7280" />
              <YAxis key="dash-status-yaxis" stroke="#6B7280" />
              <Tooltip key="dash-status-tooltip" />
              <Bar key="dash-status-bar" dataKey="value" fill="#E31937" radius={[8, 8, 0, 0]} name="Cases" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={activityData}>
              <CartesianGrid key="dash-activity-grid" strokeDasharray="3 3" stroke="#E5E7EB" />
              <XAxis key="dash-activity-xaxis" dataKey="month" stroke="#6B7280" />
              <YAxis key="dash-activity-yaxis" stroke="#6B7280" />
              <Tooltip key="dash-activity-tooltip" />
              <Legend key="dash-activity-legend" />
              <Line key="dash-activity-line-cases" type="monotone" dataKey="cases" stroke="#E31937" strokeWidth={2} name="Cases" />
              <Line key="dash-activity-line-accounts" type="monotone" dataKey="accounts" stroke="#2c3e50" strokeWidth={2} name="Accounts" />
              <Line key="dash-activity-line-projects" type="monotone" dataKey="projects" stroke="#666666" strokeWidth={2} name="Projects" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
